...
public class HashUtil {
	...
	private static final int crctab[] = { 0x00000000, 0x77073096, 0xee0e612c,
	...
	/**
	 * 1亿个数字（0～1亿），无重复数字个数:计算耗时
	 * (Xmx:8G) 97867960:201165ms
	 * (Xmx:8G,hashcode)99562592:148229ms
	 * 必须使用UTF-8编码进行中文解析，才能保证和C++版的保持一致
	 * TODO **WARNNIN** 针对男女的hash结果是一致的
	 */
	public static int crc32Hash(byte[] key) {
		int n = key.length;
		int hash = n;
		for (int i = 0; i < n; ++i){
			int pos = (hash & 0xff) ^ key[i];
			if(pos<0){
				pos = pos%crctab.length + crctab.length;
			}
			if(pos >=crctab.length){
                pos = pos%crctab.length;
            }
			hash = (hash >> 8) ^ crctab[pos ];
		}
		return hash;
	}
	...
	public static void main(String[] args) throws UnsupportedEncodingException{
		...
		//必须使用UTF-8编码进行文中解析，才能保证和C++版的保持一致
		System.out.println(HashUtil.crc32Hash(test.getBytes("utf8")) % 2);
		System.exit(0);
	}
}
